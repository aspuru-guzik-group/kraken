...
for idx,molname in enumerate(molnames):
    ...
    if os.path.exists(outfilename):
        ...
        if do:
            ...
            for c_idx in range(0,number_of_conformers[-1]):
                L_all[-1].append(data_here_confs["conf_%i"%(c_idx)]["morfeus_parameters"]["lval"])
            for c_idx in range(0,number_of_conformers[-1]):
                B1_all[-1].append(data_here_confs["conf_%i"%(c_idx)]["morfeus_parameters"]["B1"])
            for c_idx in range(0,number_of_conformers[-1]):
                B5_all[-1].append(data_here_confs["conf_%i"%(c_idx)]["morfeus_parameters"]["B5"])
